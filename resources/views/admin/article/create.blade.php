    ...
    <form class="layui-form" method="POST">
        ...
        <table class="layui-table">
            ...
            <tr>
                <td>副标题</td>
                <td>
                    <input type="text" name="subtitle" class="layui-input"  placeholder="请输副标题">
                </td>
            </tr>
            <tr>
                <td>正文</td>
                ...
                <td>
                    <select name="category_id" class="layui-select" lay-verify="required">
                        ...
                        @foreach($cateList as $cate_list)
                            @if($cate_list['p_id'] == 0)
                                <option value="{{ $cate_list['category_id'] }}">{{ $cate_list['name'] }}</option>
                                @else
                                <option value="{{ $cate_list['category_id'] }}"> ├ {{ $cate_list['name'] }}</option>
                            @endif
                        @endforeach
                    ...
                    <div class="target">
                        ...
                        <ul class="add_shopList">
                            ...
                            <li class="chose-tag">
                                <dl>
                                    @foreach($tags as $tag)
                                        <dd data-tag-id="{{ $tag->tag_id }}" id="tag-id-{{ $tag->tag_id }}">{{ $tag->name }}</dd>
                                    @endforeach
            ...
            <tr>
                <td>排序</td>
                <td>
                    <input type="number" name="sort" value="1" class="layui-input sort-input" lay-verify="required" >
                </td>
            </tr>
            <tr>
                <td>是否置顶</td>
            ...
            <tr>
                <td>是否轮播</td>
                <td>
                    <input type="checkbox" name="is_bannel" value="1" lay-skin="switch" lay-text="是|否">
                </td>
            </tr>
            <tr>
                <td>是否推荐</td>
            ...
            <tr>
                ...
                <td>
                    @foreach($type as $key=>$val)
                        <input type="radio" name="type" value="{{ $key }}" title="{{ $val }}" @if($key == \App\Models\Article::ORIGINAL) checked @endif>
                    @endforeach
                </td>
            </tr>
            </tbody>
