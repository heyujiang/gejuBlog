...
<body class="childrenBody">
    <ul class="layui-nav" lay-filter="">
        ...
        <li class="layui-nav-item layui-this"><a href="javascript:;">草稿</a></li>
    ...
    <script type="text/javascript">
        layui.use(['form', 'layer', 'jquery', 'laypage'], function () {
            ...
            function tableReload() {
                ...
                $.ajax({
                    url: "{{ url('admin/article/draft-list') }}" + "?page=" + pageConfig.pages + parameter,
                    type: "get",
                    dataType: "json",
                    success: function (res) {
                        ...
                        if (data.length != 0) {
                            for (var i = 0; i < article.length; i++) {
                                ...
                                dataHtml += '<a class="layui-btn layui-btn-xs layui-btn-normal table_release"><i class="iconfont icon-edit"></i> 发布</a>';
            ...
            $("body").on("click", ".table_edit", function () {  //编辑
                ...
                var index = layui.layer.open({
                    title: "编辑文章",
                    type: 2,
                    content: "{{ url('admin/article') }}" + "/" + id + '/edit',
                    area:['100%','100%'],
                    success: function (layero, index) {
                        setTimeout(function () {
                            layui.layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
            ...
            $("body").on("blur", ".sort-input",function () {
                ...
                if(parseInt(old_sort) != parseInt(sort)){
                    let load = layer.load();
                    $.ajax({
                        url:"{{ url('admin/article/up-sort') }}" + "/" + id,
                        type:"get",
                        data:{sort:sort},
                        dataType:'json',
             ...
             * 发布
             */
            $('body').on('click','.table_release',function () {
                let id = $(this).parents('tr').data('id');
                layer.confirm('确认发布？',function () {
                    let load = layer.load();
                    $.ajax({
                        url:"{{ url('admin/article/release') }}" + "/" + id,
                        type:"get",
                        dataAttr:"json",
                        success:function (response) {
                            if(response.code == "{{ \App\Component\Classes\Code::SUCCESS }}"){
