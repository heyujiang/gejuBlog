...
@section('body')
    ...
    <blockquote class="layui-elem-quote">
        <div class="layui-inline">
            <a class="layui-btn layui-btn-normal tableAdd_btn">添加分类</a>
        </div>
    </blockquote>
    <table class="layui-table">
        ...
        <tbody>
            @foreach($categorys as $category)
                <tr nav-id="{{ $category['category_id'] }}" data-id="{{ $category['category_id'] }}" pid={{ $category['p_id'] }} @if($category['p_id'])class="layui-hide"@endif>
                    <td>{{ $category['category_id'] }}</td>
                    <td>
                        @if($category['p_id'] == 0)
                            <i class="layui-icon x-show" status='true'>&#xe623;</i>{{ $category['name'] }}
                            @else
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├{{ $category['name'] }}
                        @endif
                    </td>
                    <td><input type="number" class="layui-input sort-input table-sort" value="{{ $category['sort'] }}"></td>
        ...
        layui.use(['form','laydate','jquery','laypage'], function () {
            ...
            $('body').on('blur','.table-sort',function () {
                ...
                if(parseInt(old_sort) != parseInt(sort)){
                    let load = layer.load();
                    $.ajax({
                        url:"{{ url('admin/category/up-sort') }}" + "/" + id,
                        type:"get",
                        data:{sort:sort},
                        dataType:'json',
                        success:function (response) {
