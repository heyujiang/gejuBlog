    ...
    <form class="layui-form" method="POST">
        ...
        <table class="layui-table">
                ...
                <td>
                    <select name="p_id" class="layui-select">
                        ...
                        @foreach($parentCategorys as $parentCategory)
                            <option value="{{$parentCategory->category_id}}" @if($parentCategory->category_id == $category->p_id) selected @endif>{{ $parentCategory->name }}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
            <tr>
                <td>导航名称</td>
                <td>
                    <input type="text" name="name" class="layui-input" value="{{ $category->name }}" lay-verify="required" placeholder="请输导航标题">
                </td>
            </tr>
            <tr>
                <td>排序</td>
                <td>
                    <input type="number" name="sort" value="{{ $category->sort }}" class="layui-input sort-input" lay-verify="required" placeholder="请输入排序">
                </td>
            </tr>
            </tbody>
        </table>
    ...
    <script>
        layui.use(['form', 'layer', 'jquery'], function () {
            ...
            form.on("submit(submit_from)", function (data) {
                $.ajax({
                    type: "put",
                    url: "{{url('admin/category',['id'=>$category->category_id])}}",
                    data: data.field,
                    async: false,
